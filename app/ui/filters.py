...
def filters(
    *,
    ...
    ss = st.session_state
    ss.setdefault(f"{state_prefix}_start_date", start_date_default)
    ss.setdefault(f"{state_prefix}_end_date", n_days_ago(1))
    ...
    filters = {
        ...
        "day_last_list": None,
        "weekday_int_list": None,
        "start_date": ss[f"{state_prefix}_start_date"],
        "end_date": ss[f"{state_prefix}_end_date"],
    }
    ...
    with st.container(border=False):
        ...
        if "pref" in visible_fields:
            with cols[col_idx]:
                filters["pref"] = st.selectbox(
                    "都道府県", fetch_prefectures(), key=f"{state_prefix}_pref"
                )
            col_idx += 1
        else:
            filters["pref"] = ss.get(f"{state_prefix}_pref", None)

        pref = filters["pref"]

        if "hall" in visible_fields:
            with cols[col_idx]:
                halls_raw = fetch_halls(pref=pref) if pref else []
                halls = halls_raw or []
                hall_sel = st.selectbox(
                    "ホール",
                    order_by_priority(halls, PRIORITY_HALLS),
                    key=f"{state_prefix}_hall",
                )
                filters["hall"] = None if hall_sel == ALL_LABEL else hall_sel
            col_idx += 1
        else:
            filters["hall"] = ss.get(f"{state_prefix}_hall", None)

        hall = filters["hall"]

        if "model" in visible_fields:
            with cols[col_idx]:
                models_raw = fetch_models(pref=pref, hall=None) if pref else []
                models = models_raw or []
                model_sel = st.selectbox(
                    "機種",
                    order_by_priority(models, PRIORITY_MODELS),
                    key=f"{state_prefix}_model",
                )
                filters["model"] = None if model_sel == ALL_LABEL else model_sel
            col_idx += 1
        else:
            filters["model"] = ss.get(f"{state_prefix}_model", None)

        model = filters["model"]

        if "start_date" in visible_fields:
            with cols[col_idx]:
        ...
        if "unit_no" in visible_fields:
            with cols[col_idx]:
                units = fetch_units(pref=pref, hall=hall, model=model) or []
                if hall is not None and units:
                    units = sorted(units)
                    filters["unit_no"] = st.multiselect(
                        "台番号",
                        units,
                        default=[units[0]],
                        key=f"{state_prefix}_unit_no",
                    ) or None
            col_idx += 1

        if "day_last_list" in visible_fields:
            with cols[col_idx]:
                DAY_LAST_LABELS = {f"{i}のつく日": i for i in range(10)}
                day_last_sel = st.multiselect(
                    "末尾日",
                    options=list(DAY_LAST_LABELS.keys()),
                    key=f"{state_prefix}_day_last_list",
                )
                filters["day_last_list"] = [
                    DAY_LAST_LABELS[label] for label in day_last_sel
                ] or None
            col_idx += 1

        if "weekday_int_list" in visible_fields:
            with cols[col_idx]:
                weekday_ja_sel = st.multiselect(
                    "曜日",
                    WEEKDAY_STR,
                    default=[],
                    key=f"{state_prefix}_weekday_int_list",
                )
                filters["weekday_int_list"] = [
                    WEEKDAY_STR_TO_INT[w] for w in weekday_ja_sel
                ] or None
            col_idx += 1
