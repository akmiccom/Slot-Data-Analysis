...
def add_prefecture_and_hall(df: pd.DataFrame, supabase: Client) -> None:
    ...
    supabase.table("prefectures").upsert(
        pref_rows, on_conflict="name").execute()
    logger.info(f"都道府県 upsert: {len(pref_rows)} 件")

    # 2) prefecture_id を取得してマップ化
    pref_res = supabase.table("prefectures").select(
        "prefecture_id, name").execute()
    ...
    if hall_rows:
        ...
        logger.info(f"ホール upsert: {len(hall_rows)} 件")
    else:
...
def add_data_result(df: pd.DataFrame, supabase: Client) -> None:
    ...
    pref_res = supabase.table("prefectures").select(
        "prefecture_id, name").execute()
    hall_res = supabase.table("halls").select(
        "hall_id, name, prefecture_id").execute()
    model_res = supabase.table("models").select("model_id, name").execute()

    pref_map = {p["name"]: p["prefecture_id"] for p in pref_res.data}
    # (pref, hall) -> hall_id
    hall_map = {(h["prefecture_id"], h["name"]): h["hall_id"]
                for h in hall_res.data}
